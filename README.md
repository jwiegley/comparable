...
It would be possible -- in fact the code has been written -- to treat variant
fields as if they were structures, and then to just compare those structures
using the same logic as for structs above. However, this has not been done yet
because enum variants very often contain fewer fields than structs, and it
would increase verbosity in the change description to name these implied
structs. For example, it would cause the above example to produce this
`Change` type:
...
struct MyEnumTwoChange {
    ...
    two_more: <u32 as Comparable>::Change,
}
```

If it turns out that users prefer this "expanded" definition for greater
consistency with the handling of structs, then that code can be activated or
made optional.
