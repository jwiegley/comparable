...
pub fn generate_describe_body_for_structs(
    ...
    match &st.fields {
        ...
        syn::Fields::Unnamed(unnamed) => {
            let field_indices =
                map_fields(false, unnamed.unnamed.iter(), |r| syn::Index::from(r.index));
            quote! {
    ...
    match field_count(true, st.fields.iter()) {
        ...
        _ => {
            ...
            let variants = match &st.fields {
                ...
                syn::Fields::Unnamed(unnamed) => {
                    map_fields(true, unnamed.unnamed.iter(), change_field)
                }
...
pub fn generate_comparison_body_for_structs(
    ...
    let (field_names_and_comparisons, field_variants): (
        Vec<(TokenStream, TokenStream)>,
        Vec<syn::Ident>,
    ) = map_fields(
        true,
        st.fields.iter(),
        |r: &FieldRef| -> ((TokenStream, TokenStream), syn::Ident) {
            ...
            (
                (name, quote!(#self_value.comparison(&#other_value))),
                variant,
            )
        },
    )
