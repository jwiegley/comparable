    ...
    match data {
        ...
        syn::Data::Union(un) => syn::Data::Union(syn::DataUnion {
            fields: syn::FieldsNamed {
                named: FromIterator::from_iter(map_fields(
                    inject_synthetics,
                    un.fields.named.iter(),
                    f,
                )),
                ..un.fields.clone()
            },
...
pub fn map_on_fields_over_datastruct(
    ...
    syn::Data::Struct(syn::DataStruct {
        fields: map_on_fields(inject_synthetics, &st.fields, f),
        ..*st
    })
}
...
pub fn map_on_fields(
    ...
    match fields {
        ...
        syn::Fields::Unnamed(unnamed) => syn::Fields::Unnamed(syn::FieldsUnnamed {
            unnamed: FromIterator::from_iter(map_fields(
                inject_synthetics,
    ...
    fields.into_iter().for_each(|field| {
        if inject_synthetics {
            if let Some(synthetics) = has_attr(&field.attrs, "comparable_synthetic").map(|attr| {
                parse_synthetics(&attr.tokens).expect(
                    "Argument to comparable_synthetic must be a set of field values in braces",
                )
            }) {
                synthetics.into_iter().for_each(|(ident, closure)| {
                    result.push(f(&FieldRef {
                        ...
                        accessor: Box::new(move |x| {
                            syn::parse2(quote!((#closure)(&#x)))
                                .expect("Could not create synthetic accessor!")
                        }),
                    }));
                    index += 1;
                });
        ...
        if has_attr(&field.attrs, "comparable_ignore").is_none() {
            result.push(f(&FieldRef {
                index,
                field,
                accessor: standard_accessor(index, field),
            }));
            index += 1;
        }
    });
...
fn parse_synthetics(
    tokens: &TokenStream,
) -> Result<BTreeMap<syn::Ident, syn::ExprClosure>, syn::Error> {
