...
impl HashMapBuilder {
	fn new() -> Self {
		HashMapBuilder { elements: Vec::new() }
	}
...
fn test_hashmap_example_new() {
	let mut map = HashMapBuilder::new().add_element(1, 100).add_element(2, 200).create();
...
#[test]
fn test_hashmap() {
	assert_changes!(&HashMap::<i32, i32>::new(), &HashMap::<i32, i32>::new(), Unchanged,);
	assert_changes!(
		&HashMap::new(),
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<HashMap<_, _>>(),
		Changed(vec![MapChange::Added(0, 1), MapChange::Added(1, 2), MapChange::Added(2, 3),]),
	);
	assert_changes!(
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<HashMap<_, _>>(),
		&HashMap::new(),
		Changed(vec![MapChange::Removed(0), MapChange::Removed(1), MapChange::Removed(2),]),
	);
	assert_changes!(
...
#[test]
fn test_btreemap() {
	assert_changes!(&BTreeMap::<i32, i32>::new(), &BTreeMap::<i32, i32>::new(), Unchanged,);
	assert_changes!(
		&BTreeMap::new(),
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		Changed(vec![MapChange::Added(0, 1), MapChange::Added(1, 2), MapChange::Added(2, 3),]),
	);
	assert_changes!(
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
	...
	assert_changes!(
		&vec![(0, 1 as i32), (1, 2)].into_iter().collect::<BTreeMap<_, _>>(),
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		Changed(vec![MapChange::Added(2, 3)]),
	);
	assert_changes!(
	...
	assert_changes!(
		&vec![(0, 1 as i32), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		Changed(vec![MapChange::Added(1, 2)]),
	);
	assert_changes!(
	...
	assert_changes!(
		&vec![(0, 1 as i32), (1, 2), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		&vec![(0, 1 as i32), (1, 4), (2, 3)].into_iter().collect::<BTreeMap<_, _>>(),
		Changed(vec![MapChange::Changed(1, I32Change(2, 4))]),
	);
}
